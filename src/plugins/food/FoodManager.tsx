...
export default function FoodManager({ open, noonItems, eveningData, onClose, onSave }: Props) {
  ...
  // ── Noon operations — call shared API immediately ──

  const handleAddNoon = async () => {
    const name = newItemName.trim()
    if (!name || noonBusy) return
    setNoonBusy(true)
    const item = await addNoonItem(name)
    if (item) {
  ...
  const confirmEdit = async () => {
    if (!editId || !editName.trim() || noonBusy) return
    setNoonBusy(true)
                  ...
                  <div key={item.id} className="flex items-center gap-xs surface-item rounded-[2px] px-sm py-2">
                    {editId === item.id ? (
                      ...
                      <>
                        ...
                        <button type="button" onClick={() => handleDeleteNoon(item.id)}
