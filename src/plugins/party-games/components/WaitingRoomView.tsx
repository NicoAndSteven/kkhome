...
const WaitingRoomView = ({ room, connectionLabel, canStart, startHint, onStart, onCopyInvite, onLeave, showModeBadge = false }: Props) => {
  ...
  return (
    <div className="space-y-4">
      {/* 主卡片 */}
      <div className="overflow-hidden rounded-[28px] border border-amber-200/60 bg-white shadow-[0_2px_24px_-8px_rgba(251,146,60,0.12)]">
        <div className="px-5 py-5">
          {/* 头部：房间码 */}
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold uppercase tracking-[0.15em] text-amber-500">房间码</span>
                {showModeBadge && (
            ...
            <button
              ...
              onClick={onLeave}
              aria-label="离开房间"
              className="rounded-xl bg-red-50 p-2.5 text-red-400 transition-all duration-200 hover:bg-red-100 active:scale-95"
            >
          ...
          {/* 游戏信息卡片 */}
          <div className="mt-5 flex items-center gap-3 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 p-4">
            <div className="flex-1">
              <span className="text-xs font-medium text-amber-500">玩家</span>
            ...
            <div className="flex-1">
              <span className="text-xs font-medium text-amber-500">模式</span>
              <strong className="mt-1 block text-base font-bold text-gray-900">
                {room.settings.mode === 'undercover' ? '谁是卧底' : '真心话大冒险'}
              </strong>
            </div>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3">
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="rounded-xl bg-white px-4 py-2 text-xs font-semibold text-amber-600 shadow-[0_1px_3px_rgba(0,0,0,0.06)] transition-all duration-200 hover:shadow-[0_2px_8px_rgba(0,0,0,0.1)] active:scale-95"
            >
              {copied ? '已复制' : '复制邀请'}
            </button>
          </div>
          ...
          <button
            ...
            onClick={onStart}
            disabled={!canStart}
            className="mt-6 w-full rounded-2xl bg-gradient-to-r from-amber-400 to-orange-500 px-5 py-3.5 text-base font-semibold text-white shadow-[0_4px_16px_-4px_rgba(251,146,60,0.4)] transition-all duration-200 hover:shadow-[0_6px_20px_-4px_rgba(251,146,60,0.55)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 disabled:active:scale-100"
          >
